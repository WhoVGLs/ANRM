...
Compact Unix VS Code monitor for a running C++ process, with CPU, memory, NVIDIA GPU, VRAM, trend charts, and diagnostics.

### Changes in 1.0 Release 
- Slimmer metric cards and charts.
- A dark grid chart inspired by Task Manager, but more modern.
- Monitor appears in its own `C++ Monitor` Activity Bar tab.
- Added `C++ Runtime Monitor: Focus Primary Sidebar View`.
- Added Marketplace packaging metadata and `.vscodeignore` for cleaner VSIX packages.

The monitor is a Webview view in the `C++ Monitor` Activity Bar container, not an editor tab or Bottom Panel.
...
Use `Ctrl+Shift+P` -> `C++ Runtime Monitor: Start`.
